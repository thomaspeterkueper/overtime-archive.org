...
const REVIEWED_AT = '2026-09-25T09:22:21+02:00';
...
const reviews = [
  {
    ...
    legacyRelations: ['OTA-TEC-0027-2091-DE', 'OTA-TEC-0028-2025-DE'],
    explicitRelations: ['OTA-SCI-0019-2025-DE', 'OTA-RED-0012-2171-DE', 'OTA-BIO-0010-2025-DE', 'OTA-TEC-0025-2091-DE'],
    sentinels: ['2030–2100', 'χ-Feld-Resonatoren', 'ε(λ,θ) = α(λ,θ)', 'fließen', '→'],
  },
  {
    ...
    legacyRelations: ['OTA-SCI-0027-2025-DE'],
    explicitRelations: ['OTA-SCI-0019-2025-DE', 'OTA-RED-0012-2171-DE', 'OTA-TEC-0029-2026-DE'],
    sentinels: ['20–800 °C', 'χ-Felder', 'Partikelgröße', 'FLÜSSIGKEITEN', '— T.P.K.'],
  },
  {
    signature: 'OTA-SCI-0034-2026-DE',
    title: 'MoM-z14 — Die älteste Galaxie',
    epistemicStatus: '["R", "T"]',
    legacyRelations: ['OTA-SCI-0044-2026-DE'],
    explicitRelations: ['OTA-RED-0012-2171-DE', 'OTA-SCI-0019-2025-DE', 'OTA-BIO-0010-2025-DE'],
    sentinels: ['MoM-z14 – Die älteste Galaxie', 'M☉', '0,6–5 µm', '±0,02', '40×'],
  },
  {
    signature: 'OTA-SCI-0030-2025-DE',
    title: 'HD 20794 d — Die episodische Supererde',
    epistemicStatus: '["R", "T", "S"]',
    legacyRelations: ['OTA-SCI-0031-2024-DE', 'OTA-SCI-0032-2024-DE'],
    explicitRelations: ['OTA-SCI-0022-2025-DE', 'OTA-RED-0012-2171-DE', 'OTA-BIO-0010-2025-DE'],
    sentinels: ['außergewöhnlich', 'Instituto de Astrofísica de Canarias', 'M⊕', '0,40–0,45', '≈'],
  },
  {
    ...
    legacyRelations: ['OTA-SCI-0044-2026-DE', 'OTA-SCI-0036-2026-DE'],
    explicitRelations: ['OTA-RED-0022-MULTI-DE', 'OTA-SCI-0035-2026-DE', 'OTA-SCI-0019-2025-DE'],
    sentinels: ['[S] 100% — Spekulatives AVI-Framework', 'noχ¹ᐃ-Welt', '0–5', 'Übersicht', '10⁶ cm⁻³'],
  },
];
...
function recoverOnePass(text) {
  ...
  for (let i = 0; i < chars.length; i += 1) {
    ...
    try {
      ...
      // Keep source sequence when reverse byte mapping is not valid UTF-8.
    }
...
function patchMetadata(raw, review) {
  ...
  const targets = [...review.legacyRelations, ...review.explicitRelations];
  if (inline) {
    ...
    fm = fm.replace(/^relatedDocuments:\s*\[[^\n]*\]\s*$/m, relationYaml(review));
    changed = true;
  } else {
    const relationBlock = fm.match(/^relatedDocuments:\s*\n((?:^[ \t].*(?:\r?\n|$))*)/m);
    if (!relationBlock) throw new Error(`${review.signature}: relatedDocuments is neither reviewed legacy list nor normalized block`);
...
console.log(`Encoding batch 1: ${reviews.length} documents, ${totalRepairs} encoding sequence(s), ${reviews.reduce((sum, review) => sum + review.explicitRelations.length, 0)} promoted explicit relations.`);
